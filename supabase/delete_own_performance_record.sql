-- Standalone copy of migrations/011_delete_own_performance_record.sql
-- Run in Supabase Dashboard → SQL Editor (prod). Idempotent.
--
-- Account-specific delete fix:
--   ON DELETE SET NULL on leaderboard_shares.source_record_id fires an UPDATE
--   on inactive share rows. That used to re-require a Leaderboard Name, so
--   deletes failed for accounts without a name (often test accounts) while
--   working for accounts that still have one.
